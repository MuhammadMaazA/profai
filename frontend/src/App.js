...
function App() {
  ...
  const sendVoiceMessage = async (audioBlob) => {
    ...
    try {
      ...
      // Add AI response - use audio_url from backend (already generated)
      const aiMessage = {
        type: 'ai',
        content: aiResponse,
        audioUrl: audio_url ? `${API_BASE_URL}/audio/${audio_url}` : null,
        emotion: detected_emotion
      };
      ...
      setMessages(prev => {
        ...
        // Auto-play the audio if available and not muted
        if (audio_url && !isAudioMuted) {
          const audio = new Audio(`${API_BASE_URL}/audio/${audio_url}`);
          audio.play().catch(console.error);
        }
  ...
  const sendTextMessage = async () => {
    ...
    try {
      ...
      // Add AI response - use audio_path from backend if available
      const aiMessage = {
        type: 'ai',
        content: answer,
      ...
      setMessages(prev => {
        ...
        // If no audio_path from backend, then auto-generate audio for the new AI message
        if (!audio_path && !isAudioMuted) {
          autoGenerateAudio(answer, messageIndex);
        } else if (audio_path && !isAudioMuted) {
          // Auto-play the audio that was already generated by backend
